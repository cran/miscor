...
#
# Internal function: Validate.correlation
#
# Function copied from the PoisNonNor package <cran.r-project.org/web/packages/PoisNonNor>
internal.Validate.correlation <- function (cmat, pmat = NULL, lamvec = NULL) {
  ...
  if (!internal.is.positive.definite(cmat)) {
  ...
  for (i in 2:(n1 + n2)) {
    for (j in 1:(i - 1)) {
      if (cmat[i, j] > maxcor[i, j] | cmat[i, j] < mincor[i, j]) {
        cat("\n cmat[", i, ",", j, "] must be between ",
            round(mincor[i, j], 3), " and ", round(maxcor[i, j], 3), "\n")
